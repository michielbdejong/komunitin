...
export function setupServerTest(createData: boolean = true, graftCreditCommons: boolean = false): TestSetupWithCurrency {
  const test = {
    ...
    ccNeighbour: { peerNodePath: 'trunk', lastHash: 'trunk' },
    ...
    createCreditCommonsNeighbour: async (peerNodePath: string, lastHash: string, vostroId: string, admin = userAuth("0")) => {
      await test.api?.post('/TEST/creditCommonsNodes', testCreditCommonsNeighbour(peerNodePath, lastHash, vostroId), admin)
    }
  }
  ...
  before(async () => {
    ...
    if (createData) {
      ...
      if (graftCreditCommons) {
        console.log("Setting up neighbour", test.ccNeighbour.peerNodePath, test.ccNeighbour.lastHash, test.account0.id)
        await test.createCreditCommonsNeighbour(test.ccNeighbour.peerNodePath, test.ccNeighbour.lastHash, test.account0.id)
      }
    }
  })
